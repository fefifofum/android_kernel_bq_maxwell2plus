...
#define RK_CAM_FRAME_INVAL_INIT 1
...
struct rk_camera_dev
{
    ...
    int icd_width;
    int icd_height;
     enum rk_cif_scale_mode scale_mode;
...
static void rk_camera_set_scale_mode(struct soc_camera_device *icd,int scale_mode)
{
   struct soc_camera_host *ici =to_soc_camera_host(icd->dev.parent);
    struct rk_camera_dev *pcdev = ici->priv;
 if(scale_mode)
      	{
	  	pcdev->scale_mode = Cif_Scale_Directly;
		printk("hognhaishen Cif_Scale_Crop \n");
      	}
	  else
	  	{
	  	pcdev->scale_mode = Cif_Scale_Crop;
		printk("hognhaishen Cif_Scale_Directly \n");
	  	}
...
if (pcdev->scale_mode == Cif_Scale_Crop)
{
                                ...
                                struct v4l2_capability *cap)
{
    ...
    for (i=0; i<RK_CAM_NUM;i++) {
        if ((pcdev->pdata->info[i].dev_name!=NULL) && (strcmp(dev_name(pcdev->icd->pdev), pcdev->pdata->info[i].dev_name) == 0)) {
	      ...
	      sprintf(fov,"_%d_%d",pcdev->pdata->register_dev[i].link_info.fov_v,pcdev->pdata->register_dev[i].link_info.fov_h);
        }
    }
...
static struct soc_camera_host_ops rk_soc_camera_host_ops =
{
    ...
    .controls = rk_camera_controls,
     .set_scale_mode = rk_camera_set_scale_mode,
